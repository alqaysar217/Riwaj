...
export default function AddProductPage() {
  ...
  return (
    <div className="min-h-full bg-muted/40 pb-28">
      <div className="container mx-auto px-4 py-8 space-y-10 max-w-6xl">
        {/* Header with High Contrast */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[35px] border shadow-sm relative overflow-hidden group">
          ...
          <div className="flex items-center gap-5 relative z-10">
            <Button variant="ghost" size="icon" onClick={() => router.back()} className="w-12 h-12 rounded-2xl bg-muted/20 border-none text-primary shrink-0 hover:bg-primary/5 transition-all">
          ...
          <div className="flex gap-3 relative z-10">
             ...
             <Button onClick={handleSave} className="rounded-2xl h-14 bg-primary hover:bg-primary/90 shadow-xl shadow-primary/20 gap-3 px-10 font-bold transition-all hover:scale-[1.02] active:scale-95 text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div 
                className={cn(
                  "relative aspect-square rounded-[45px] border-4 border-dashed transition-all duration-500 overflow-hidden shadow-md group cursor-pointer",
                  image ? "border-primary/20 bg-white" : "border-muted-foreground/10 bg-white hover:border-primary/40 hover:bg-primary/5"
                )}
              ...
              >
                {image ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center space-y-4">
                    <div className="w-20 h-20 bg-muted/30 rounded-[28px] flex items-center justify-center text-primary shadow-inner group-hover:rotate-6 transition-transform duration-500">
            ...
            {/* AI Assistance Card - Distinct Gradient */}
            <Card className="border-none shadow-2xl shadow-secondary/10 rounded-[45px] bg-gradient-to-br from-[#D4A017] to-[#B48812] p-8 relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-2xl bg-white shadow-md flex items-center justify-center">
                    <Sparkles className="w-6 h-6 text-[#D4A017] animate-pulse" />
                  </div>
                  <div>
                    <h3 className="font-headline font-bold text-lg text-white">ذكاء رواج للتاجر</h3>
                    <p className="text-[10px] text-white/70 font-bold uppercase tracking-wider">مساعدك الشخصي</p>
                  </div>
                </div>
                ...
                <Button 
                  ...
                  className="w-full bg-white hover:bg-white/90 text-[#D4A017] font-bold h-14 rounded-2xl gap-3 shadow-xl transition-all hover:scale-[1.02] active:scale-95"
                >
                  {isGenerating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5 fill-[#D4A017]" />}
          ...
          {/* Right Column: Form Details - High Contrast White Cards */}
          <div className="lg:col-span-8 space-y-8">
            {/* Main Info Card */}
            <Card className="border-none shadow-xl shadow-black/[0.04] rounded-[45px] overflow-hidden bg-white">
              <CardHeader className="bg-muted/30 border-b p-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="font-bold text-muted-foreground/70">أدخل تفاصيل المنتج بدقة لضمان سهولة وصول العملاء إليه</CardDescription>
                  </div>
                </div>
              </CardHeader>
            ...
            <Card className="border-none shadow-xl shadow-black/[0.04] rounded-[45px] overflow-hidden bg-white">
              <CardHeader className="bg-muted/30 border-b p-10">
                ...
                <CardDescription className="font-bold text-muted-foreground/70 pr-14">أضف كلمات مفتاحية تساعد العملاء في الوصول لمنتجك بسهولة</CardDescription>
              </CardHeader>
            ...
            <div className="flex flex-col sm:flex-row gap-5 pt-6 pb-12">
              <Button onClick={handleSave} className="flex-[3] h-20 rounded-[30px] bg-primary hover:bg-primary/90 text-2xl font-headline font-bold shadow-2xl shadow-primary/30 gap-4 group transition-all hover:scale-[1.02] active:scale-95 text-white">
                <Save className="w-7 h-7 group-hover:scale-110 transition-transform" /> حفظ ونشر المنتج في السوق
              </Button>
              <Button variant="ghost" onClick={() => router.push('/merchant/products')} className="flex-1 h-20 rounded-[30px] font-bold text-muted-foreground gap-3 bg-white shadow-sm hover:bg-destructive/5 hover:text-destructive transition-all text-lg border border-black/[0.02]">
